...
1. Check `output/{brand}/{product}/research/` — does `04_master_doc.md` and `06_desire_validation.md` exist?
   - No → tell user to run `/deep-research "{brand}" "{product}"` first (research phase must complete first)
   - Yes → proceed

2. Load `output/{brand}/{product}/research/product-marketing-context.md` as shared brand context.
...
5. Save outputs to `output/{brand}/{product}/research/` (overwrites existing files for selected steps).
...
2. `output/{brand}/{product}/research/product-marketing-context.md` — avatar, desires, proof points
